...
<DocAccordion buttonContent="*Universal Profiling*">
...
For more on Universal Profiling, refer to the <DocLink slug="/en/observability/universal-profiling">Universal Profiling</DocLink> docs.

![Host Universal Profiling](../images/universal-profiling-overlay.png)
...
to filter by. For more information, refer to <DocLink slug="/en/observability/configure-data-sources" section="customize-stream-page">Customize Stream</DocLink>.
...
[Anomaly Explorer in ((ml-app))](((ml-docs))/ml-gs-results.html). You can also select **Actions -> Show in Inventory** to view the host
...
that includes the [Osquery Manager](((integrations-docs))/osquery_manager.html)
integration and have Osquery [((kib)) privileges](((kibana-ref))/kibana-privileges.html) as a user.
