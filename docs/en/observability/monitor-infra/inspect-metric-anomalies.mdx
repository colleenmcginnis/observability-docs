---
id: enObservabilityInspectMetricAnomalies
slug: /en/observability/inspect-metric-anomalies
...
1. You’ll be prompted to create a ((ml)) job for **Hosts** or
    **Kubernetes Pods**. Click **Enable**.

1. Choose a start date for the ((ml)) analysis.
...
1. By default, ((ml)) jobs analyze all of your metric data, and the results are listed under
...
1. You're now ready to explore your metric anomalies. Click **Anomalies**.
    ...
    to view the [Anomaly Explorer in ((ml-app))](((ml-docs))/ml-gs-results.html). You can
    also select **Actions -> Show in Inventory** to view the host or Kubernetes pods Inventory
    page, filtered by the specific metric.

    <DocCallOut title="Note">

    These predefined ((anomaly-jobs)) use [custom rules](((ml-docs))/ml-rules.html). To
    update the rules in the [Anomaly Explorer](((ml-docs))/ml-gs-results.html), select
...
[Anomaly Explorer](((ml-docs))/ml-gs-results.html).
