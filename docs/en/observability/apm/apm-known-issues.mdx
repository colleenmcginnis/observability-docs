---
id: enObservabilityApmKnownIssues
slug: /en/observability/apm-known-issues
title: Known issues
description: Description to be written
tags: []
---

<div id="apm-known-issues"></div>
...
<div id="broken-apm-anomaly-rule"></div>
...
1. Filter your rules by setting **Type** to **APM Anomaly**.
1. For each anomaly rule in the list, select the pencil icon to edit the rule.
1. Add one or more **DETECTOR TYPES** to the rule.
...
1. Find broken rules
    ...
    * `fields=[id, name, actions, tags, schedule, notify_when, throttle, params]`
    ...
    <DocAccordion buttonContent="Example result:">
...
1. Prepare the update JSON doc(s)
    ...
    1. Add the `"anomalyDetectorTypes"` to the `"params"` block, using the default value as seen below to mimic the pre-8.13 behavior:
...
1. Update each rule using the `PUT /api/alerting/rule/{id}` API

    For each rule, submit a PUT request to the [update rule endpoint](((kibana-ref))/update-rule-api.html) using that rule's ID and its stored update document from the previous step. For example, assuming the first broken rule's ID is `046c0d4f`:
...
<div id="apm-empty-metricset-values"></div>
...
Find details in [elastic/apm-data#157](https://github.com/elastic/apm-data/pull/157).
