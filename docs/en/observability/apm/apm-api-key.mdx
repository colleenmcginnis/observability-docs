---
slug: /en/observability/apm-api-key
title: API keys
description: Description to be written
tags: []
---

import ApiKeyWidget from './tab-widgets/api-key-widget.mdx'
import ApikeySubcommands from './command-reference/apikey-subcommands.mdx'

<div id="apm-api-key"></div>

<DocCallOut title="Important" color="warning">
API keys are sent as plain-text,
so they only provide security when used in combination with <DocLink slug="/en/observability/apm-agent-tls">TLS</DocLink>.
</DocCallOut>
...
* **Agent configuration** (`config_agent:read`): Required for agents to read
    [Agent configuration remotely](((kibana-ref))/agent-configuration.html).
...
make sure <DocLink slug="/en/observability/apm-agent-tls">TLS</DocLink> is enabled, then complete these steps:

1. <DocLink slug="/en/observability/apm-api-key" section="enable-api-keys">Enable API keys</DocLink>
1. <DocLink slug="/en/observability/apm-api-key" section="create-an-api-key-user-in-((kib))">Create an API key user</DocLink>
1. <DocLink slug="/en/observability/apm-api-key" section="create-an-api-key-in-the-((apm-app))">Create an API key in ((kib))</DocLink>
1. <DocLink slug="/en/observability/apm-api-key" section="set-the-api-key-in-your-apm-agents">Set the API key in your APM agents</DocLink>

<div id="apm-enable-api-key"></div>

## Enable API keys

<ApiKeyWidget />

<div id="apm-create-api-key-user"></div>
...
The example below uses the ((kib)) [role management API](((kibana-ref))/role-management-api.html)
...
<div id="apm-create-an-api-key"></div>
...
![((apm-app)) API key](images/apm-ui-api-key.png)

<div id="apm-agent-api-key"></div>
...
* **Android**: [`apiKey`](((apm-android-ref))/configuration.html)
* **Go agent**: [`ELASTIC_APM_API_KEY`](((apm-go-ref))/configuration.html#config-api-key)
* **.NET agent**: [`ApiKey`](((apm-dotnet-ref))/config-reporter.html#config-api-key)
* **iOS**: [`withApiKey`](((apm-ios-ref))/configuration.html#withApiKey)
* **Java agent**: [`api_key`](((apm-java-ref))/config-reporter.html#config-api-key)
* **Node.js agent**: [`apiKey`](((apm-node-ref))/configuration.html#api-key)
* **PHP agent**: [`api_key`](((apm-php-ref-v))/configuration-reference.html#config-api-key)
* **Python agent**: [`api_key`](((apm-py-ref))/configuration.html#config-api-key)
* **Ruby agent**: [`api_key`](((apm-ruby-ref))/configuration.html#config-api-key)

<div id="apm-configure-api-key-alternative"></div>

## Alternate API key creation methods

API keys can also be created and validated outside of ((kib)):

* <DocLink slug="/en/observability/apm-api-key" section="apm-server-api-key-workflow">APM Server API key workflow</DocLink>
* <DocLink slug="/en/observability/apm-api-key" section="((es))-api-key-workflow">((es)) API key workflow</DocLink>

<div id="apm-create-api-key-workflow-apm-server"></div>
...
<div id="apm-create-api-key-subcommands"></div>

#### `apikey` subcommands

<ApikeySubcommands />

<div id="apm-create-api-key-privileges"></div>
...
<div id="apm-create-api-key-workflow"></div>
...
A full list of `apikey` subcommands and flags is available in the <DocLink slug="/en/observability/apm-command-line-options" section="apikey-command">API key command reference</DocLink>.

<div id="apm-create-api-key-workflow-es"></div>

### ((es)) API key workflow

It is also possible to create API keys using the ((es))
[create API key API](((ref))/security-api-create-api-key.html).
...
[Authenticate API](((ref))/security-api-authenticate.html):
