...
Refer to [Manual setup with `-javaagent` flag](((apm-java-ref-v))/setup-javaagent.html) to learn more.
...
* **Automatic setup with `apm-agent-attach-cli.jar`**
    ...
    or JVM options, and allows attaching to a running JVM. Refer to the [Java agent documentation](((apm-java-ref-v))/setup-attach-cli.html) for more information on this setup method.
...
* **Programmatic API setup to self-attach**
    ...
    the agent artifact is embedded within the packaged application binary. Refer to the [Java agent documentation](((apm-java-ref-v))/setup-attach-api.html) for more information on this setup method.
