---
id: enObservabilityApmMonitoringMetricbeatCollection
slug: /en/observability/apm-monitoring-metricbeat-collection
...
<div id="apm-monitoring-metricbeat-collection"></div>
...
1. <DocLink id="enObservabilityApmMonitoringMetricbeatCollection" section="configure-the-shipper-you-want-to-monitor">Configure the shipper you want to monitor</DocLink>

1. <DocLink id="enObservabilityApmMonitoringMetricbeatCollection" section="install-and-configure-((metricbeat))-to-collect-monitoring-data">Install and configure ((metricbeat)) to collect monitoring data</DocLink>

<div id="apm-configure-shipper"></div>
...
1. Disable the default collection of APM Server monitoring metrics.
...
1. Configure cluster UUID (optional).
...
<div id="apm-configure-metricbeat"></div>

## Install and configure ((metricbeat)) to collect monitoring data

export let apm_server = false

1. Install ((metricbeat)) on the same server as APM Server. To learn how, see
...
1. Configure the `beat-xpack` module in ((metricbeat)).
    ...
    To monitor multiple<DocIf condition={ apm_server === false }>((beats)) agents,</DocIf><DocIf condition={ apm_server === true }>APM Server instances,</DocIf>
...
1. Identify where to send the monitoring data.
    ...
    <DocCallOut title="Tip">
    If you're using ((ilm)), the remote monitoring user
    requires additional privileges to create and read indices. For more
    information, see <DocLink id="enObservabilityApmSecureCommsStack" section="feature-roles">Feature roles</DocLink>.
    </DocCallOut>
