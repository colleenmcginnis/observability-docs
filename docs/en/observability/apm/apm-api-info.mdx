...
the instructions at <DocLink id="enObservabilityApmApiKey">APM API key</DocLink> or <DocLink id="enObservabilityApmSecretToken">APM Secret Token</DocLink>,
must be followed to configure the correct permissions for APM access.

If an <DocLink id="enObservabilityApmApiKey">API keys</DocLink> or a <DocLink id="enObservabilityApmSecretToken">Secret token</DocLink> is passed along with
...
Example APM Server information request with POST, with a <DocLink id="enObservabilityApmSecretToken">Secret token</DocLink>:
