---
id: enObservabilityApmSetupKibanaEndpoint
slug: /en/observability/apm-setup-kibana-endpoint
title: Configure the ((kib)) endpoint
...
<div id="apm-setup-kibana-endpoint"></div>
...
<ApmSupportOverview binary={true} fleet={false}>
  ...
  Please see <DocLink id="enObservabilityApmConfigureAgentConfig">Configure APM agent configuration</DocLink> instead.
</ApmSupportOverview>
...
<div id="((kib))_endpoint_configuration_options"></div>
...
When a node is defined as an `IP:PORT`, the _scheme_ and _path_ are taken
from the <DocLink id="enObservabilityApmSetupKibanaEndpoint" section="apm-serverkibanaprotocol">apm-server.kibana.protocol</DocLink> and
<DocLink id="enObservabilityApmSetupKibanaEndpoint" section="apm-serverkibanapath">apm-server.kibana.path</DocLink> config options.
</DocCallOut>

IPv6 addresses must be defined using the following format:
`https://[2001:db8::1]:5601`.

<div id="apm-kibana-protocol-option"></div>
...
<div id="`apm-server.kibana.ssl.enabled`"></div>
...
see <DocLink id="enObservabilityApmConfigurationSsl">SSL/TLS output settings</DocLink>.
