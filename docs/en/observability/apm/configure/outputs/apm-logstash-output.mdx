---
slug: /en/observability/apm-logstash-output
title: Configure the ((ls)) output
...
import SharedSslLogstashConfig from '../../apm-configuring-ssl-logstash.mdx'

<div id="apm-logstash-output"></div>
...
<div id="send_events_to_((ls))"></div>

## Send events to ((ls))

To send events to ((ls)), you must:

1. <DocLink slug="/en/observability/apm-logstash-output" section="((ls))-output-configuration">Enable the ((ls)) output in APM Server</DocLink>
1. <DocLink slug="/en/observability/apm-logstash-output" section="((kib))-endpoint-configuration">Enable the ((kib)) endpoint in APM Server</DocLink>
1. <DocLink slug="/en/observability/apm-logstash-output" section="((ls))-configuration-pipeline">Create a ((ls)) configuration pipeline in ((ls))</DocLink>

<div id="apm-ls-output-config"></div>
...
1. Enable the ((ls)) output by uncommenting the ((ls)) section and setting `enabled` to `true`:
...
<div id="apm-ls-kib-config"></div>

### ((kib)) endpoint configuration

<SharedKibanaConfig />

<div id="apm-ls-config-pipeline"></div>
...
1. Use the [Elastic Agent input plugin](((logstash-ref))/plugins-inputs-elastic_agent.html) to configure
...
1. Use the [((es)) output plugin](((logstash-ref))/plugins-outputs-elasticsearch.html) to send
    ...
    [^2]: This example assumes you're sending data to ((ecloud)). If you're using a self-hosted version of ((es)), use `hosts` instead. See [((es)) output plugin](((logstash-ref))/plugins-outputs-elasticsearch.html) for more information.
...
<div id="accessing_the_@metadata_field"></div>

## Accessing the @metadata field

Every event sent to ((ls)) contains a special field called
[`@metadata`](((logstash-ref))/event-dependent-configuration.html#metadata) that you can
...
<DocLink slug="/en/observability/apm-logstash-output" section="index">`index`</DocLink> option in the APM Server config file.
...
[event types](((observability-guide))/apm-data-model.html), namespaces, or datasets.
...
[^5]: In this example, `cloud_id` and `cloud_auth` are stored as [environment variables](((logstash-ref))/environment-variables.html)
...
<DocLink slug="/en/observability/apm-logstash-output" section="proxy_use_local_resolver">`proxy_use_local_resolver`</DocLink> option.
...
<div id="apm-logstash-proxy-use-local-resolver"></div>
...
<DocCallOut title="Note">
This parameter's value will be assigned to the `metadata.beat` field. It
...
<DocLink slug="/en/observability/apm-configuration-ssl">SSL/TLS output settings</DocLink> for more information. To use SSL, you must also configure the
[((beats)) input plugin for ((ls))](((logstash-ref))/plugins-inputs-beats.html) to use SSL/TLS.
