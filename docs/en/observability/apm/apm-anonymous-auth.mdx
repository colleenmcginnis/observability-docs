---
id: enObservabilityApmAnonymousAuth
...
|---|---|
| An <DocLink id="enObservabilityApmApiKey">API key</DocLink> or <DocLink id="enObservabilityApmSecretToken">secret token</DocLink> is configured | Anonymous requests are rejected and an authentication error is returned. |
...
You can only enable and configure anonymous authentication if an <DocLink id="enObservabilityApmApiKey">API key</DocLink> or
<DocLink id="enObservabilityApmSecretToken">secret token</DocLink> is configured. If neither are configured, these settings will be ignored.
