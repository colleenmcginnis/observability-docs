...
* `client.ip`: The client's IP address. Typically derived from the HTTP headers of incoming requests.
    ...
    see <DocLink slug="/en/observability/apm-anonymous-auth" section="deriving-an-incoming-requests-clientip-address">Deriving an incoming request's `client.ip` address</DocLink>.
