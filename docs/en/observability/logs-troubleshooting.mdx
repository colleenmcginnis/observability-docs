...
* Get an API key from an administrator and manually add the API to the ((agent)) configuration. See <DocLink slug="/en/observability/logs-stream" section="step-3-configure-the-((agent))">Configure the ((agent))</DocLink> for more on manually updating the configuration and adding the API key.
...
The host needs access to ((kib)). Port `443` must be open and the deployment's ((es)) endpoint must be reachable. Locate your project's endpoint from **Help menu (<DocImage flatImage alt="" url="images/help-icon.png" />) → Connection details**.
