---
id: enObservabilityMonitorAwsCloudtrailFirehose
slug: /en/observability/monitor-aws-cloudtrail-firehose
title: Monitor CloudTrail logs
description: Description to be written
tags: []
---

<div id="monitor-aws-cloudtrail-firehose"></div>
...
<div id="firehose-cloudtrail-prerequisites"></div>
...
<div id="firehose-cloudtrail-step-one"></div>

## Step 1: Install AWS integration in ((kib))

1. In ((kib)), navigate to **Management** > **Integrations** and browse the catalog to find the Amazon Data Firehose integration.

1. Navigate to the **Settings** tab and click **Install Amazon Data Firehose assets**.

<div id="firehose-cloudtrail-step-two"></div>

## Step 2: Export Cloudtrail events to CloudWatch

![Cloudtrail to CloudWatch](images/firehose-cloudtrail-cloudwatch.png)
...
1. Click **Create trail** and configure the general details on the **Choose trail attributes** panel, like:
...
1. Specify the encryption options.
...
1. Enable **CloudWatch Logs** and confirm the **Log group name**.
...
1. From the **Choose log events** panel, select the event types you want to send to Elastic.

1. Review the attributes and log events you have specified in the previous steps and click **Create trail**.

1. Verify everything is working as expected.
    ...
    ![Verify events in CloudWatch](images/firehose-verify-events-cloudwatch.png)

<div id="firehose-cloudtrail-step-three"></div>

## Step 3: Set up a Firehose delivery stream

![Firehose delivery stream](images/firehose-delivery-stream.png)

You now have a CloudWatch log group with events coming from CloudTrail.
For more information on how to set up a Amazon Data Firehose delivery stream to send data to Elastic Cloud, you can also check the <DocLink id="enObservabilityMonitorAwsFirehose">setup guide</DocLink>.
...
1. Set up the delivery stream by specifying the following data:
...
<div id="firehose-cloudtrail-step-four"></div>

## Step 4: Set up a subscription filter to route Cloudtrail events to a delivery stream

![Firehose subscription filter](images/firehose-subscription-filter.png)
...
1. Visit the log group with the CloudTrail events.
    ...
    [subscription filter](https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/Subscriptions.html) that allows you to choose log events from the log group and forward them to other services like Amazon Kinesis stream, an Amazon Data Firehose stream, or AWS Lambda.

1. Create a subscription filter for Amazon Data Firehose by following these steps.
    ...
    1. Grant permission.
        ...
        1. Assign a new IAM policy to the role that permits ”putting records” into a
...
![Firehose failed delivery stream](images/firehose-failed-delivery-stream.png)

The Amazon Data Firehose delivery stream reports the number of failed deliveries and failure details.

<div id="firehose-cloudtrail-step-five"></div>
...
![Firehose monitor CloudTrail logs](images/firehose-monitor-cloudtrail-logs.png)

Navigate to ((kib)) and choose among the following monitoring options:

- **Visualize your logs with Discover**

![Visualize CloudTrail logs with Disocver](images/firehose-cloudtrail-discover.png)

- **Visualize your logs with Logs explorer**

![Visualize CloudTrail logs with Logs explorer](images/firehose-cloudtrail-logsexplorer.png)

- **Visualize your logs with the CloudTrail Dashboard**

![Visualize CloudTrail logs with CloudTrail Dashboard](images/firehose-cloudtrail-dashboard.png)
