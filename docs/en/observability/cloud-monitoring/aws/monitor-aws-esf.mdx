---
id: enObservabilityMonitorAwsEsf
slug: /en/observability/monitor-aws-esf
title: Monitor Amazon Web Services (AWS) with Elastic Serverless Forwarder
description: Description to be written
tags: []
---

<div id="monitor-aws-esf"></div>

The Elastic Serverless Forwarder (ESF) is an Amazon Web Services (AWS) Lambda function that ships logs from your AWS environment to Elastic. Elastic Serverless Forwarder is published in the AWS Serverless Application Repository (SAR). For more information on ESF, check the [Elastic Serverless Forwarder Guide](((esf-ref))/aws-elastic-serverless-forwarder.html).

<div id="aws-esf-what-you-learn"></div>
...
<div id="aws-esf-prerequisites"></div>
...
1. Specify the AWS region in which you want it deployed.
...
1. Select the network interface you want to use.
1. From the **Actions** drop-down menu, choose **Create flow log**.
1. For **Destination**, select **Send to an S3 bucket**.
1. For **S3 bucket ARN**, enter the name of the S3 bucket you created in the previous step.
...
1. In the **Advanced** settings, modify the JSON access policy to define which S3 ObjectCreated events should be sent to the queue.
...
1. Go to the properties of the S3 bucket containing the VPC flow logs and enable event notification.
...
1. Create an encoded API key for authentication.
...
1. Upload the configuration file you have just created to the S3 bucket you created at step 5.

<div id="esf-step-seven"></div>
...
1. From the **Serverless application** tab, select **elastic-serverless-forwarder**.
1. On the **Review, configure and deploy** page, fill in the following fields:
