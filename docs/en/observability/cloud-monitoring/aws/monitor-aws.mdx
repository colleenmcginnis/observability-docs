---
id: enObservabilityMonitorAws
slug: /en/observability/monitor-aws
...
<div id="aws-before-you-begin"></div>
...
In the [AWS S3 console](https://s3.console.aws.amazon.com/s3), click on *Create
bucket*. Give the bucket a **name** and specify the **region** in which you want it
deployed.

![S3 bucket creation](../../images/creating-a-s3-bucket.png)

<div id="aws-step-two"></div>
...
created. The [((filebeat)) S3 input](((filebeat-ref))/filebeat-input-aws-s3.html)
...
queue whenever new logs are present in the S3 bucket. Go to the [SQS console](https://eu-central-1.console.aws.amazon.com/sqs/)
...
![Queue Creation](../../images/creating-a-queue.png)
...
![Event Notification Setting](../../images/configure-event-notification.png)

Set the destination as the SQS queue you just created.

![Event Notification Setting](../../images/configure-notification-output.png)

<div id="aws-step-four"></div>
...
<div id="aws-step-five"></div>

## Step 5: Configure the AWS Module

Now that the output is working, you can set up the
[((filebeat)) AWS](((filebeat-ref))/filebeat-module-aws.html) module which
...
make sure to edit the policy of your bucket as shown in [step 3 of the AWS user guide](https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/S3ExportTasksConsole.html).
...
![Current Architecture](../../images/one-bucket-archi.png)
...
![Architecture with Access Logging Enabled](../../images/two-buckets-archi.png)
...
![Enabling Server Access Logging](../../images/Server-Access-Logging.png)
...
![EC2 logs in the Logs UI](../../images/EC2-logs.png)
...
![S3 Server Access Log Overview](../../images/S3-Server-Access-Logs.png)

This gives you an overview of how your S3 buckets are being accessed.

<div id="aws-step-seven"></div>
...
Please see [AWS API requests](((metricbeat-ref))/metricbeat-module-aws.html#aws-api-requests) for more details.

</DocCallOut>

<div id="aws-step-eight"></div>
...
<div id="aws-step-nine"></div>

## Step 9: Configure the AWS module

To collect metrics from your AWS infrastructure, we'll use the
[((metricbeat)) AWS](((metricbeat-ref))/metricbeat-module-aws.html) module. This module
...
1. Edit the `modules.d/aws.yml` file with the following configurations.
...
![Your EC2 Infrastructure](../../images/EC2-instances.png)
