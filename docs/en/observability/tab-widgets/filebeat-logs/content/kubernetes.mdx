...
2. Follow the [Run ((filebeat)) on Kubernetes](((filebeat-ref))/running-on-kubernetes.html) guide.

3. Enable [hints-based autodiscover](((filebeat-ref))/configuration-autodiscover-hints.html) (uncomment the corresponding section in `filebeat-kubernetes.yaml`).
