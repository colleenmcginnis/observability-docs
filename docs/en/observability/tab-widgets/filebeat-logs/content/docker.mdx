...
2. Follow the [Run ((filebeat)) on Docker](((filebeat-ref))/running-on-docker.html) guide.

3. Enable [hints-based autodiscover](((filebeat-ref))/configuration-autodiscover-hints.html).

<DocIf condition={ props.ecs_logs === true }>

4. Add these labels to your containers that log using ECS-compatible JSON. This will make sure the logs are parsed appropriately.
...
<DocIf condition={ props.plaintext === true }>((empty))</DocIf>
