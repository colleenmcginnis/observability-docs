...
* The *{eui-icon-beaker} Run test manually* button that allows you to run the test on
...
* The *{eui-icon-pencil} Edit monitor* button that allows you to edit the monitor's
  configuration.
...
Use the calendar icon ({eui-icon-calendar}) and search bar
...
* The latest run of the full journey by clicking *{eui-icon-inspect} View test run*
...
* An individual step in this run by clicking the performance breakdown icon
  ({eui-icon-apmTrace}) next to one of the steps.
...
Navigate through each step using *{eui-icon-arrowLeft} Previous* and
*Next {eui-icon-arrowRight}*.
...
Click the {eui-icon-arrowRight} icon next to the step number to show details.
...
After clicking the performance breakdown icon ({eui-icon-apmTrace})
...
higher ({eui-icon-sortUp}),
lower ({eui-icon-sortDown}),
or the same ({eui-icon-minus})
...
click the {eui-icon-popout} icon located beside each resource,
