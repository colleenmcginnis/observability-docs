---
id: enObservabilityMonitorJavaApp
slug: /en/observability/monitor-java-app
title: Monitor a Java application
description: Description to be written
tags: []
---

import InstallFilebeatInstallWidgetFilebeat from './shared/copied-from-beats/install-filebeat/install-widget-filebeat.mdx'
import StartFilebeatStartWidgetFilebeat from './shared/copied-from-beats/start-filebeat/start-widget-filebeat.mdx'
import InstallMetricbeatInstallWidgetMetricbeat from './shared/copied-from-beats/install-metricbeat/install-widget-metricbeat.mdx'
import StartMetricbeatStartWidgetMetricbeat from './shared/copied-from-beats/start-metricbeat/start-widget-metricbeat.mdx'
import InstallHeartbeatInstallWidgetHeartbeat from './shared/copied-from-beats/install-heartbeat/install-widget-heartbeat.mdx'
import StartHeartbeatStartWidgetHeartbeat from './shared/copied-from-beats/start-heartbeat/start-widget-heartbeat.mdx'

<div id="monitor-java-app"></div>
...
Prometheus Module](((metricbeat-ref))/metricbeat-module-prometheus.html) and view your metrics in ((kib)).
- Instrument your application using the [Elastic APM
Java agent](((apm-java-ref))/index.html).
...
Create a deployment using our hosted ((ess)) on [((ecloud))](((ess-trial))). The deployment includes
...
[observability-contrib
GitHub repository](https://github.com/elastic/observability-contrib/tree/main/monitor-java-app) for the sample application.

<div id="step_1:_create_a_java_application"></div>

## Step 1: Create a Java application

To create the Java application, you require OpenJDK 14 (or higher) and the [Javalin](https://javalin.io/)
...
1. Install the Gradle wrapper. An easy way to install Gradle is to use
    [sdkman](https://sdkman.io/) and run `sdk install gradle 6.5.1`. Next run
    `gradle wrapper` in the current directory to install the Gradle wrapper.

1. Run `./gradlew clean check`. You should see a
    successful build that has nothing built or compiled yet.

1. To create a Javalin server and its first endpoint (the main endpoint), create the
...
1. In the `build.gradle` file, edit `plugins` as shown here.
...
1. Run `./gradlew shadowJar`. This command creates a
...
1. Rebuild the project and start the server.
...
1. Test the code. Placing everything into the `main()` method makes it difficult to test
...
1. After the tests pass, build and package the application.

    ```bash
    ./gradlew clean check shadowJar
    ```

<div id="step_2:_ingest_logs"></div>
...
<div id="add_logging_implementation"></div>
...
1. To start logging, edit the `App.java` file and change a handler.
...
1. Create a log4j2 configuration in the `src/main/resources/log4j2.xml` file. You might need to create that directory first.
...
1. Rebuild and restart the application. The log messages are logged for each
    request.
...
<div id="create_an_iso8601_timestamp"></div>
...
<div id="log_to_a_file_and_stdout"></div>
...
1. Restart the application and send a request. The logs will be sent to
    `/tmp/javalin/app.log`.

<div id="install_and_configure_((filebeat))"></div>
...
<InstallFilebeatInstallWidgetFilebeat />

1. Use the ((filebeat)) keystore to store [secure
    settings](((filebeat-ref))/keystore.html). Let’s store the Cloud ID in the keystore.
...
1. Log into ((kib)) user (you can do so from the Cloud Console without typing
...
1. To load the ((filebeat)) dashboards, use the `elastic` super user.
...
1. Configure ((filebeat)), so it knows where to read data from and where to send it
...
<div id="send_data_to_((es))"></div>

### Send data to ((es))

To send data to ((es)), start ((filebeat)).

<StartFilebeatStartWidgetFilebeat
...
like [wrk](https://github.com/wg/wrk) and run the following command to send requests to the application.
...
<div id="step_3:_view_logs_in_((kib))"></div>
...
1. Log into ((kib)) and select the **Discover** app.
    ...
    ![((kib)) single document view](images/monitor-java-app-kibana-single-document.png)
...
1. Now let's have a look at the ((logs-app)) in ((kib)). Select **((observability))** -> **Logs**.
...
1. To view a continuous stream of log messages, click **Stream live**. You can also
    highlight specific terms, as shown here.

    ![((kib)) Logs UI Streaming](images/monitor-java-app-kibana-streaming.png)
...
<div id="step_4:_work_with_your_logs"></div>

## Step 4: Work with your logs

<div id="structure_logs"></div>
...
sending it to ((es)) by using [processors](((filebeat-ref))/filtering-and-enhancing-data.html).
If the capabilities of these processors are not enough, you can always let ((es)) do the heavy lifting by using
[an ingest node](((ref))/ingest.html). This is what many modules in ((filebeat)) do. A module in ((filebeat))
...
[script
processor](((filebeat-ref))/processor-script.html) and call `trim()`. However, it might be easier to fix our logging
...
<div id="configure_log_rotation"></div>
...
the [Simulate Pipeline API](((ref))/simulate-pipeline-api.html).

1. Let’s write a pipeline that is similar to our ((filebeat)) processors using
...
1. Now, on to the ((filebeat)) configuration. First, let’s remove all the
    processors, except the
    [add_host_metadata
    processor](((filebeat-ref))/add-host-metadata.html), to add some host information like the host name and operating
    system.
...
1. Edit the ((es)) output to ensure the pipeline will be
...
1. Restart ((filebeat)) and see if logs are flowing in as expected.

<div id="write_logs_as_json"></div>
...
[ecs-logging-java](https://github.com/elastic/ecs-logging-java). The advantage of
ECS logging is that it uses the [Elastic Common
Schema](((ecs-ref))/index.html). ECS defines a standard set of fields used when storing event data in
((es)), such as logs and metrics.

1. Instead of writing our logging standard, use an existing one. Let’s add
    ...
    ```xml
    <RollingFile name="JavalinAppLogRolling" fileName="/tmp/javalin/app.log" filePattern="/tmp/javalin/%d{yyyy_MM_dd}-%i.log.gz">
...
1. You can configure a few
    [more
    parameters](https://github.com/elastic/ecs-logging-java/tree/main/log4j2-ecs-layout) for the `EcsLayout`, but the defaults have been selected wisely. Let’s
...
<div id="step_5:_ingest_metrics"></div>
...
we will use the [Metricbeat
Prometheus Module](((metricbeat-ref))/metricbeat-module-prometheus.html) to send data to ((es)).

The underlying library used in our app is
[micrometer.io](http://micrometer.io/), a vendor-neutral application
metrics facade in combination with its
[Prometheus support](http://micrometer.io/docs/registry/prometheus) to
implement a pull-based model. You could use the
[elastic support](http://micrometer.io/docs/registry/elastic) to achieve
...
<div id="add_metrics_to_the_application"></div>
...
1. Add the micrometer plugin and its corresponding import to our Javalin app.
...
1. Add a new metrics endpoint and ensure the `BasicAuthCredentials` class is
    imported as well.
...
1. Rebuild your app and poll the metrics endpoint.
...
1. Similar to the ((filebeat)) setup, run the initial set up of all the dashboards
...
1. Store the combination of `id` and `api_key` fields in the keystore.
...
1. Configure ((metricbeat)) to read our Prometheus metrics. Start with a
...
1. To improve security, you should add the username and the
    password to the keystore and refer to both in the configuration.

1. Start ((metricbeat)).

<StartMetricbeatStartWidgetMetricbeat
  modulename={undefined}
/>
...
<div id="step_6:_view_metrics_in_((kib))"></div>
...
1. Create a line chart and select `metricbeat-*` as the source.

    The basic idea is to have a
    [max
    aggregation](((ref))/search-aggregations-metrics-max-aggregation.html) on the y-axis on the `prometheus.log4j2_events_total.rate`
    field, whereas the x-axis, is split by date using a
    [date_histogram
    aggregation](((ref))/search-aggregations-bucket-datehistogram-aggregation.html) on the `@timestamp` field.

    There is one more split within
    each date histogram bucket, split by log level, using a
    [terms
    aggregation](((ref))/search-aggregations-bucket-terms-aggregation.html) on the `prometheus.labels.level`, which contains the log
    level. Also, increase the size of the log level to six to display
    every log level.

    The final result looks like this.

    ![Date Histogram of the log rate per log level](images/monitor-java-app-metrics-kibana-create-visualization-log-rate.png)]

<div id="visualize_open_files_over_time"></div>
...
1. Add an endpoint to increase the number of open files by
...
1. Let’s run a `wrk` workload.
    ...
    [Lens](((kibana-ref))/dashboard.html#create-panels-with-lens) in ((kib)).

    ![Lens visualization](images/monitor-java-app-metrics-kibana-create-visualization-open-files.png)]

1. Below `Add filter`, select the `metricbeat-*` index
...
1. Now let's have a look at the ((infrastructure-app)) in ((kib)). Select **((observability))** -> **Infrastructure**.
...
1. Click **Metrics Explorer**, you can
    start exploring your data for specific hosts or the CPU usage across your
    nodes.

    ![Metrics UI Log Counter](images/monitor-java-app-metrics-ui-prometheus-event-counter.png)]
...
<div id="step_7:_instrument_the_application"></div>
...
![A transaction with spans](images/monitor-java-app-apm-transactions.png)
...
[APM agent
documentation](((apm-agents-ref))/index.html).

<div id="add_the_((apm-agent))_to_your_code"></div>
...
[for
the most recent version](https://search.maven.org/search?q=g:co.elastic.apm%20AND%20a:elastic-apm-agent).
...
<div id="automatic_attachment"></div>
...
on the
[official
docs](((apm-java-ref))/setup-attach-cli.html).
...
1. Instrument the application right at the start in our `main()` method.
    ...
    [documentation](((apm-java-ref))/setup-attach-api.html#setup-attach-api-configuration)
    ...
    repository. Mechanisms like [vault](https://www.vaultproject.io/) allow
    you to manage your secrets in such a way.

    For our local deployment, I usually use something like
    [direnv](https://direnv.net/) for local setup. `direnv` is an extension
...
1. To enable `direnv`, you need to create a `.envrc` file with this.
    ...
    file, you can use tools like [envchain](https://github.com/sorah/envchain)
    or call arbitrary commands in the `.envrc` file like accessing Vault.

1. You can now run the java application as you did before.
    ...
    ![Javalin App APM UI](images/monitor-java-app-apm-ui-javalin-app.png)
...
<div id="custom_transactions"></div>
...
1. Fix the name of the transactions to include the HTTP method
...
1. Restart your app and see data flowing in. Test a few different
    endpoints, especially the one that throws exceptions and the one
    that triggers a 404.

    ![APM UI with correct transaction names](images/monitor-java-app-apm-ui-javalin-with-transaction-names.png)]

    This looks much better, having differences between endpoints.

1. Add another endpoint to see the power of transactions, which polls
    another HTTP service. You may have heard of [wttr.in](https://wttr.in/), a
    ...
    [Apache
    HTTP client](https://hc.apache.org/httpcomponents-client-4.5.x/quickstart.html), one of the most typical HTTP clients out there.
...
1. Curl `http://localhost:7000/weather/Munich` and
    ...
    ![Overview](images/monitor-java-app-apm-ui-javalin-wttr-1.png)
    ...
    ![Transaction with span](images/monitor-java-app-apm-ui-javalin-wttr-2.png)
...
<div id="method_tracing_via_agent_configuration"></div>
...
The agent can
[trace
methods](((apm-java-ref))/config-core.html#config-trace-methods) based on their signature.
...
1. Fix the call in our `App` class.

    ```java
    config.requestLogger(new Log4j2RequestLogger());
    ```

1. Restart your app, and see how much time your logging takes.

    ![Logging caller trace](images/monitor-java-app-apm-ui-logging-trace.png)
...
<div id="automatic_profiling_of_inferred_spans"></div>
...
[automatic
profiling of inferred spans](((apm-java-ref))/config-profiling.html#config-profiling-inferred-spans-enabled) by setting the following.
...
[Elastic
APM OpenTracing bridge](((apm-java-ref))/opentracing-bridge.html) or looked into the
[additional
metrics](((apm-java-ref))/metrics.html) the agent provides, which allows us to take a look at things
like garbage collection or the memory footprint of our application.

</DocCallOut>

<div id="step_8:_ingest_uptime_data"></div>
...
[Uptime](https://www.elastic.co/uptime-monitoring) not only enables you to monitor the availability
...
<InstallHeartbeatInstallWidgetHeartbeat />
...
1. Let’s setup the ((heartbeat)) keystore and run the setup.
...
1. Add some services to monitor.
...
1. Now start ((heartbeat)) and wait a couple of minutes to get some data.

<StartHeartbeatStartWidgetHeartbeat
  modulename={undefined}
/>
...
![Uptime Overview](images/monitor-java-app-uptime-overview.png)
...
((observability)), see the <DocLink id="enObservabilityObservabilityIntroduction">((observability)) documentation</DocLink>.
