---
slug: /en/observability/logs-filter-and-aggregate
...
* <DocLink slug="/en/observability/logs-filter-and-aggregate" section="filter-logs">Filter logs</DocLink> — Narrow down your log data by applying specific criteria.
* <DocLink slug="/en/observability/logs-filter-and-aggregate" section="aggregate-logs">Aggregate logs</DocLink> — Analyze and summarize data to find patterns and gain insight.

<div id="logs-filter-and-aggregate-prereq"></div>

## Before you get started

The examples on this page use the following ingest pipeline and index template, which you can set in **Dev Tools**. If you haven't used ingest pipelines and index templates to parse your log data and extract structured fields yet, start with the <DocLink slug="/en/observability/logs-parse">Parse and organize logs</DocLink> documentation.
...
- <DocLink slug="/en/observability/logs-filter-and-aggregate" section="filter-logs-in-log-explorer">Filter logs in Log Explorer</DocLink> – Filter and visualize log data in ((kib)) using Log Explorer.
- <DocLink slug="/en/observability/logs-filter-and-aggregate" section="filter-logs-with-query-dsl">Filter logs with Query DSL</DocLink> – Filter log data from Dev Tools using Query DSL.
...
From Log Explorer, you can use the [((kib)) Query Language (KQL)](((kibana-ref))/kuery-query.html) in the search bar to narrow down the log data displayed in Log Explorer.
...
1. Click the current time range, select **Absolute**, and set the **Start date** to `Sep 14, 2023 @ 00:00:00.000`.
    ...
    ![Set the start date for your time range, 50%](images/logs-start-date.png)

1. Click the end of the current time range, select **Absolute**, and set the **End date** to `Sep 15, 2023 @ 23:59:59.999`.

    ![Set the end date for your time range, 50%](images/logs-end-date.png)

Under the **Documents** tab, you'll see the filtered log data matching your query.

![Filter data by log level using KQL](images/logs-kql-filter.png)
