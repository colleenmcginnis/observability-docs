...
((heartbeat)) and <DocLink slug="/en/observability/uptime-get-started">configured with a traditional `heartbeat.yml` file</DocLink>.
...
<DocLink slug="/en/observability/monitor-uptime-synthetics">((synthetics-app))</DocLink> instead of the ((uptime-app)).
...
Monitor the availability of network endpoints and services using <DocLink slug="/en/observability/monitor-uptime-synthetics" section="lightweight-https-tcp-and-icmp-monitors">Lightweight HTTP/S, TCP, and ICMP monitors</DocLink>.
...
To set up your first monitor, refer to <DocLink slug="/en/observability/uptime-get-started">Get started with Uptime</DocLink>.
...
an assigned status is derived from the threshold values in the <DocLink slug="/en/observability/configure-uptime-settings">Settings</DocLink> page.
...
<DocLink slug="/en/observability/tls-certificate-alert">TLS rule</DocLink> to receive an alert when your certificate is about to expire.
