---
slug: /en/observability/profiling-self-managed-running
title: "Step 4: Run the backend applications"
description: Description to be written
tags: []
---

<div id="profiling-self-managed-running"></div>

The next step is to run the backend applications. To do this:

1. <DocLink slug="/en/observability/profiling-self-managed-running" section="create-api-keys">Create API keys</DocLink> to authenticate the backend applications.
1. Run the application on <DocLink slug="/en/observability/profiling-self-managed-running" section="run-on-linux">Linux</DocLink> or <DocLink slug="/en/observability/profiling-self-managed-running" section="run-on-kubernetes">Kubernetes</DocLink>.

<div id="profiling-self-managed-create-api-keys"></div>
...
Refer to [Create an API key](((kibana-ref))/api-keys.html#create-api-key) to create an API key using ((kib)).
...
Continue to <DocLink slug="/en/observability/profiling-self-managed-running" section="run-on-linux">Run on Linux</DocLink> or <DocLink slug="/en/observability/profiling-self-managed-running" section="run-on-kubernetes">Run on Kubernetes</DocLink> for information on running the backend applications.

<div id="profiling-self-managed-running-linux"></div>

## Run on Linux

Before running the backend applications on Linux, we recommend creating <DocLink slug="/en/observability/profiling-self-managed-running" section="create-configuration-files">configuration files</DocLink> to manage the applications.
...
1. <DocLink slug="/en/observability/profiling-self-managed-running" section="os-packages-debrpm">OS packages (DEB/RPM)</DocLink>
1. <DocLink slug="/en/observability/profiling-self-managed-running" section="oci-containers">OCI containers</DocLink>
1. <DocLink slug="/en/observability/profiling-self-managed-running" section="binary">Binary</DocLink>: orchestrated with your configuration management system of choice (Ansible, Puppet, Chef, Salt, etc.)

<div id="profiling-self-managed-running-linux-configfile"></div>
...
<div id="profiling-self-managed-running-linux-configfile-collector"></div>
...
Customize the content of the `output.elasticsearch` section, using the Elasticsearch endpoint and <DocLink slug="/en/observability/profiling-self-managed-running" section="create-api-keys">API key</DocLink> to set the `hosts` and `api_key` values, respectively.
Adjust the `protocol` value and other TLS related settings as needed.

<DocAccordion buttonContent="Collector configuration file">
...
<div id="profiling-self-managed-running-linux-configfile-symbolizer"></div>
...
<DocAccordion buttonContent="Symbolizer configuration file">
...
<div id="profiling-self-managed-install-os-packages"></div>

### OS packages (DEB/RPM)

Follow these steps to install the backend using OS packages.

<div id="profiling-run-backend-deb"></div>
...
<div id="profiling-run-backend-rpm"></div>
...
1. Install the backend services by running:
...
Refer to <DocLink slug="/en/observability/profiling-self-managed-troubleshooting">Troubleshooting Universal Profiling backend</DocLink> for more information on troubleshooting possible errors in the logs.

<div id="profiling-self-managed-running-linux-container"></div>
...
<div id="profiling-self-managed-running-linux-binary"></div>
...
1. Copy the `pf-elastic-collector` and `pf-elastic-symbolizer` binaries to a directory in the machine's `PATH`.
1. Run the backend application processes, instructing them to read the configuration files created previously.
...
<div id="profiling-self-managed-running-kubernetes"></div>
...
For the applications' configuration, you can reuse the configuration files detailed in <DocLink slug="/en/observability/profiling-self-managed-running" section="create-configuration-files">"Create configuration files"</DocLink> and pass them to Helm as a values file (using the `--values` of `-f` flags), or copy them in the `values.yaml` file.
...
1. Install the charts (we are using the `universal-profiling` namespace, but you can customize at will):
...
1. Check the pods are running and read their logs, by following the steps listed in the output of the `helm install` commands.

<DocCallOut title="Note">
In the previous examples, we used the charts' default values to configure Kubernetes resources. These **do not** include the creation of an `Ingress` resource.
...
Continue to <DocLink slug="/en/observability/profiling-self-managed-install-next-steps">Step 5: Next steps</DocLink>.
