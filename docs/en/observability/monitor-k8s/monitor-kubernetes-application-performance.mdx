...
import Widget from '../shared/install-apm-agents-kube/widget.mdx'

<div id="monitor-kubernetes-application-performance"></div>
...
<DocAccordion buttonContent="Expand alternatives">

* [((ecloud)) on Kubernetes (ECK)](((eck-ref))/) -- The Elastic recommended approach for managing
...
* [Download and install APM Server](((observability-guide))/apm-getting-started-apm-server.html) -- The classic, non-Kubernetes option.

</DocAccordion>

<div id="step_2:_save_your_secret_token"></div>

### Step 2: Save your secret token

A [secret token](((observability-guide))/apm-secret-token.html) is used to secure communication between APM agents
...
see [secret token](((observability-guide))/apm-secret-token.html) for instructions on how to set up your secret token.

If you are using ECK to set up APM Server, the operator automatically generates an `{APM_server_name}-apm-token` secret for you.

<div id="step_3:_install_and_configure_apm_agents"></div>
...
<div id="step_4:_configure_kubernetes_data"></div>
...
These environment variable are set via the [Downward API](https://kubernetes.io/docs/tasks/inject-data-application/environment-variable-expose-pod-information/#the-downward-api)
...
<div id="step_5:_deploy_your_application"></div>

### Step 5: Deploy your application

APM agents are deployed with your application.

<DocAccordion buttonContent="Resource configuration file example">
...
<div id="view_your_traces_in_((kib))"></div>
...
![((apm-app)) Kubernetes](../images/apm-app-landing.png)
...
![((apm-app)) Kubernetes](images/apm-app-kubernetes-filter.png)
