...
import InstallFilebeatInstallWidgetFilebeat from './copied-from-beats/install-filebeat/install-widget-filebeat.mdx'

<div id="install_((filebeat))"></div>
...
As a best practice, [use an administrator role to set up](((filebeat-ref))/privileges-to-setup-beats.html)
and a more restrictive role for event publishing (which you will do next).

</DocCallOut>

<div id="configure_((filebeat))_output"></div>

# Configure ((filebeat)) output

Next, you are going to configure ((filebeat)) output to ((ess)).

1. Use the ((filebeat)) keystore to store [secure settings](((filebeat-ref))/keystore.html).
...
1. To store logs in ((es)) with minimal permissions, create an API key to send
    ...
    details [here](((filebeat-ref))/feature-roles.html).
...
1. To configure ((filebeat)) to output to ((ess)), edit the `filebeat.yml`
