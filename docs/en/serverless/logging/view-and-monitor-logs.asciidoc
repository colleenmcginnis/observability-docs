...
Update this setting by going to *Management* → *Advanced Settings* and searching for *logs source*.
...
For more on filtering your data in Logs Explorer, refer to <<filter-and-aggregate-logs-filter-logs-in-logs-explorer,Filter logs in Logs Explorer>>.
...
* **Filter out value (image:images/icons/minusInCircle.svg[filter out value icon]):** Show logs that do *not* contain the specific field value.
