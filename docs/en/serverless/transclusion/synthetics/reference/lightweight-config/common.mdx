...
(list of <<synthetics-lightweight-string,string>>s)
...
**`status.enabled`** (<<synthetics-lightweight-boolean,boolean>>)::
...
**`tls.enabled`** (<<synthetics-lightweight-boolean,boolean>>)::
...
or via the CLI using the <<synthetics-command-reference-elasticsynthetics-push,`--location` flag on `push`>>.
...
* Run the <<synthetics-command-reference-elasticsynthetics-locations,`elastic-synthetics locations` command>> and specify the URL of the Observability project. This will fetch all available private locations associated with the deployment.
...
or via the CLI using the <<synthetics-command-reference-elasticsynthetics-push,`--privateLocations` flag on `push`>>.
...
<<synthetics-analyze-synthetics-analyze-individual-monitors-overview,individual monitor's *Overview* tab>>.
